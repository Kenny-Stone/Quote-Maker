...
def addQuoteToVideo(video : VideoEditor) -> None:
    ...
    quotes = quotes.getResults()[randint(0,len(quotes - 1))]    # get quotes with author name and gets a random index
    quoteFormat = f"{quotes["quote"]}\n- {quotes["author"]}"        # formats quote
    ...
    video.addText(
        ...
        _color="#ffffff",
        size=(1080 - (50 + 160 + PADDING_FROM_IMAGE),835)
    )
...
def main():
    try:
        ...
        video = VideoDownloader(
            ...
            destination="videos",verbose=True
            )
        ...
        mainVideo.saveVideo("video1.mp4",threads = 4)
    finally:
